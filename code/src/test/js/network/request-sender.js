...
const isRetryNeeded = (rawResponse, retryTime) => {
  ...
  let errorCode = rawResponse.data.errCode;
  if (errorCode === 10006) {
  ...
  if (errorCode === 10010 && !lodash.isNil(rememberMeToken)) {
    // Permission Denied but No need to sign in
    return true;
  }
...
export default class RequestSender {
   ...
   * @returns the actual payload if success, @example { "name": "mitrade" }
   *
